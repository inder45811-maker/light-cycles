...
export default function AgentBuilder() {
  ...
  const [marketplace, setMarketplace] = useState<Agent[]>([])
  ...
  useEffect(() => { loadAgents(); loadMarketplace() }, [])
  ...
  const loadMarketplace = async () => {
    try { setMarketplace(await api.get<Agent[]>('/api/agents/marketplace')) } catch {}
  }
  ...
  return (
    <div style={{ maxWidth: 700 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <h2 className="section-title" style={{ margin: 0 }}>Your Agents</h2>
        <div style={{ display: 'flex', gap: 8 }}>
          <button className={view === 'mine' ? 'btn btn-primary' : 'btn btn-sm'}
            style={{ borderColor: 'var(--border)' }}
            onClick={() => setView('mine')}>My Agents</button>
          <button className={view === 'market' ? 'btn btn-primary' : 'btn btn-sm'}
            style={{ borderColor: 'var(--border)' }}
            onClick={() => setView('market')}>Marketplace</button>
        </div>
      </div>
            ...
            <div className="arena-panel" style={{ padding: 24, marginBottom: 24 }}>
              ...
              <div style={{ marginTop: 16 }}>
                ...
                <textarea className="form-input" rows={3} value={form.custom_prompt}
                  ...
                  placeholder="Extra instructions for your agent's behavior..." />
              </div>
      ...
      {view === 'market' && (
        <>
          <div style={{ color: 'var(--text-dim)', fontSize: 12, marginBottom: 20 }}>
            Community agents you can use in tournaments and battles.
          </div>
            ...
            marketplace.map(agent => (
              <div key={agent.id} className="arena-panel" style={{ padding: 20, marginBottom: 12 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div>
                    <div style={{ fontFamily: 'var(--font-display)', fontSize: 14, color: 'var(--cyan-bright)' }}>{agent.name}</div>
                    <div style={{ color: 'var(--text-dim)', fontSize: 11, marginTop: 4 }}>{agent.description}</div>
                  </div>
                  <div style={{ fontSize: 10, color: 'var(--text-dim)', fontFamily: 'var(--font-mono)', textAlign: 'right' }}>
                    <div>{agent.personality}</div>
                    <div>{agent.mode === 'webhook' ? 'Webhook' : 'AI'}</div>
                  </div>
                </div>
                {agent.stats && <div style={{ display: 'flex', gap: 12, marginTop: 8, fontSize: 9, color: 'var(--text-dim)' }}>
                  <span>⚔️ {agent.stats.wins || 0}W</span>
                  <span>🏆 {agent.stats.tournament_wins || 0}T</span>
                </div>}
              </div>
            ))
          )}
